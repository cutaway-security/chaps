...
########## Create storage directory for files in user's Temp directory at $env:temp ##############
...
Catch{
    ...
    Catch{
        $err_str + "IPv4 Network Settings check failed" | Tee-Object -FilePath $out_file -Append
    }
}
...
$inf_str + "Testing if PowerShell Commandline Auditing is Enabled" | Tee-Object -FilePath $out_file -Append
Try{
...
Try{
    if ([bool](Get-ItemProperty -path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ModuleLogging' -ErrorAction SilentlyContinue).EnableModuleLogging){
        $pos_str + "PowerShell EnableModuleLogging Is Set" | Tee-Object -FilePath $out_file -Append
    } else {
        $neg_str + "PowerShell EnableModuleLogging Is Not Set" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
    Try{
    ...
    Catch{
        $err_str + "Testing PowerShell EnableModuleLogging failed" | Tee-Object -FilePath $out_file -Append
    }
}
...
Try{
    if ([bool](Get-ItemProperty -path 'HKLM:\Software\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging' -ErrorAction SilentlyContinue).EnableScriptBlockLogging){
        $pos_str + "PowerShell EnableScriptBlockLogging Is Set" | Tee-Object -FilePath $out_file -Append
    } else {
        $neg_str + "PowerShell EnableScriptBlockLogging Is Not Set" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
    Try{
...
Try{
    if ([bool](Get-ItemProperty -path 'HKLM:\Software\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging' -ErrorAction SilentlyContinue).EnableScriptBlockInvocationLogging){
        $pos_str + "PowerShell EnableScriptBlockInvocationLogging Is Set" | Tee-Object -FilePath $out_file -Append
    } else {
        $neg_str + "PowerShell EnableScriptBlockInvocationLogging Is Not Set" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
    Try{
...
Try{
    if ([bool](Get-ItemProperty -path 'HKLM:\Software\Policies\Microsoft\Windows\PowerShell\Transcription' -ErrorAction SilentlyContinue).EnableTranscripting){
        $pos_str + "PowerShell EnableTranscripting Is Set" | Tee-Object -FilePath $out_file -Append
    } else {
        $neg_str + "PowerShell EnableTranscripting Is Not Set" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
    Try{
...
Try{
    if ([bool](Get-ItemProperty -path 'HKLM:\Software\Policies\Microsoft\Windows\PowerShell\Transcription' -ErrorAction SilentlyContinue).EnableInvocationHeader){
        $pos_str + "PowerShell EnableInvocationHeader Is Set" | Tee-Object -FilePath $out_file -Append
    } else {
        $neg_str + "PowerShell EnableInvocationHeader Is Not Set" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
    Try{
...
Try{
    if ([bool](Get-ItemProperty -path 'HKLM:\Software\Policies\Microsoft\Windows\EventLog\ProtectedEventLogging' -ErrorAction SilentlyContinue).EnableProtectedEventLogging){
        $pos_str + "PowerShell EnableProtectedEventLogging Is Set" | Tee-Object -FilePath $out_file -Append
    } else {
        $neg_str + "PowerShell EnableProtectedEventLogging Is Not Set" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
    Try{
...
$inf_str + "Event log size default settings are too small. Test that max sizes have been increased." | Tee-Object -FilePath $out_file -Append
...
Try{
    if ([bool](Get-ItemProperty -path "HKLM:\System\CurrentControlSet\Control\Terminal Server" -ErrorAction SilentlyContinue).AllowRemoteRPC){
        $neg_str + "AllowRemoteRPC should be set to disable RDP" | Tee-Object -FilePath $out_file -Append
    } else {
        $pos_str + "AllowRemoteRPC is set to deny RDP" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
...
Try{
    if ([bool](Get-ItemProperty -path "HKLM:\System\CurrentControlSet\Control\Terminal Server" -ErrorAction SilentlyContinue).fDenyTSConnections){
        $pos_str + "fDenyTSConnections is set to deny remote connections" | Tee-Object -FilePath $out_file -Append
    } else {
        $neg_str + "fDenyTSConnections should be set to not allow remote connections" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
...
Catch{
    $err_str + "Testing if Windows Network Firewall rules allow remote connections failed" | Tee-Object -FilePath $out_file -Append
}
...
Try{
    ...
    foreach ($n in (Get-LocalGroupMember -Group "Administrators").Name) {
        $rep_str + "Account in local Administrator group: $n" | Tee-Object -FilePath $out_file -Append
    }
}
...
Try{
    if ([bool](Get-ItemProperty -path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\policies\system' -ErrorAction SilentlyContinue).LocalAccountTokenFilterPolicy){
        $neg_str + "PowerShell LocalAccountTokenFilterPolicy Is Set" | Tee-Object -FilePath $out_file -Append
    } else {
        $pos_str + "PowerShell LocalAccountTokenFilterPolicy Is Not Set" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
    $err_str + "Testing for PowerShell LocalAccountTokenFilterPolicy in Policies failed" | Tee-Object -FilePath $out_file -Append
}
...
Try{
    if ([bool](Get-ItemProperty -path 'HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\policies\system' -ErrorAction SilentlyContinue).LocalAccountTokenFilterPolicy){
        $neg_str + "PowerShell LocalAccountTokenFilterPolicy in Wow6432Node Is Set" | Tee-Object -FilePath $out_file -Append
    } else {
        $pos_str + "PowerShell LocalAccountTokenFilterPolicy in Wow6432Node Is Not Set" | Tee-Object -FilePath $out_file -Append
    }
}
Catch{
    $err_str + "Testing for PowerShell LocalAccountTokenFilterPolicy in Wow6432Node failed." | Tee-Object -FilePath $out_file -Append
}
...
########## Report Information ##############
$rep_str + "Completed Date/Time: $(get-date -format yyyyMMddTHHmmssffzz)" | Tee-Object -FilePath $out_file -Append
