...
function Get-CredDeviceGuard {
    ...
    if ([System.Environment]::OSVersion.Version.Major -ge 10){
        Try{
            $secServConfig = (Get-WmiObject –ClassName Win32_DeviceGuard –Namespace root\Microsoft\Windows\DeviceGuard).SecurityServicesConfigured
            $secServRunning = (Get-WmiObject –ClassName Win32_DeviceGuard –Namespace root\Microsoft\Windows\DeviceGuard).SecurityServicesRunning
        }
        Catch{
...
function Get-AntiVirus {
    ...
    Try{
        $avProducts = Get-WmiObject -Namespace root\SecurityCenter2 -ClassName AntiVirusProduct -ErrorAction Stop
